...
echo "test message" | ./ucat -S once "$socket" || clean_and_exit $hard_fail

# Give listener time to process and exit
sleep 0.2
wait $listener_pid 2>/dev/null || true

# Should only see the message, no credential info (Linux behavior)
expected="test message"
r=$(cat "$results")
case "$r" in
    "$expected") ;;
    *)
        echo "expected '$expected', actual was '$r'"
        clean_and_exit $fail
        ;;
esac
